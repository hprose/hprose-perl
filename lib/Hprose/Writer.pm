...
sub new {
    ...
    if ($simple) {
        ...
        $refer->{set} = sub {
            my ($self, $val) = @_;
            $self->{refer}->{ref}->{$val} = $self->{refer}->{refcount}++;
        };
        $refer->{write} = sub {
            my ($self, $val) = @_;
            my $ref = $self->{refer}->{ref};
            if (exists($ref->{$val})) {
        ...
        $refer->{reset} = sub {
            my $self = shift;
            undef %{$self->{refer}->{ref}};
            $self->{refer}->{refcount} = 0;
        };
...
sub write_string_with_ref {
    my ($self, $val) = @_;
    $self->{refer}->{write}($self, $val) or $self->write_string($val);
}
...
sub write_bytes_with_ref {
    my ($self, $val) = @_;
    $self->{refer}->{write}($self, $val) or $self->write_bytes($val);
}
...
sub write_date_with_ref {
    my ($self, $val) = @_;
    $self->{refer}->{write}($self, $val) or $self->write_date($val);
}
...
sub write_guid_with_ref {
    my ($self, $val) = @_;
    $self->{refer}->{write}($self, $val) or $self->write_guid($val);
}
...
sub write_array_with_ref {
    my ($self, $val) = @_;
    $self->{refer}->{write}($self, $val) or $self->write_array($val);
}
...
sub write_hash_with_ref {
    my ($self, $val) = @_;
    $self->{refer}->{write}($self, $val) or $self->write_hash($val);
}
...
sub write_object {
    ...
    exists($fieldsCache{$classname}) or $fieldsCache{$classname} = [keys(%$val)];
    my $fields = $fieldsCache{$classname};
    my $classref = $self->{classref};
    exists($classref->{$classname}) or $self->write_class($classname, $fields);
    my $index = $classref->{$classname};
    $self->{refer}->{set}($self, $val);
...
sub write_object_with_ref {
    my ($self, $val) = @_;
    $self->{refer}->{write}($self, $val) or $self->write_object($val);
}
